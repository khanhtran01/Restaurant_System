<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <!-- <meta name="viewport" content="width=device-width, initial-scale=1.0"> -->
    <link rel="stylesheet" href="./../../assets/css/base-food.css">
    <link rel="stylesheet" href="./../../assets/css/main-food.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/normalize/8.0.1/normalize.css"
        integrity="sha512-oHDEc8Xed4hiW6CxD7qjbnI+B07vDdX7hEPTvn9pSZO1bcRqHp8mj9pyr+8RVC2GmtEfI2Bi9Ke9Ass0as+zpg=="
        crossorigin="anonymous" referrerpolicy="no-referrer" />
    
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.3.0/font/bootstrap-icons.css">
    <title>Document</title>
</head>

<body>
    <div class="app">
        <div class="header">
            <div class="grid">
                <div class="grid__row">
                    <div class="nav_bar_up">
                        <div class="nav_bar_up_info">
                            <div class="nav_bar_up_info_telephone">
                                <i class="bi bi-phone"></i>
                                <div>0392747972</div>
                            </div>
                            <div class="nav_bar_up_info_email">
                                <i class="bi bi-envelope"></i>
                                <div>bku@hcmut.edu.vn</div>
                            </div>
                            <div class="nav_bar_up_info_location">
                                <i class="bi bi-geo-alt"></i>
                                <div>Địa chỉ</div>
                            </div>
                        </div>
                        <div class="nav_bar_up_login">
                            <a href="">Đăng nhập</a>
                            <a href="">Đăng kí</a>
                        </div>
                    </div>
                </div>
                <div class="grid_row">
                    <div class="nav_bar_down">
                        <div class="nav_bar_up_logo">
                            <img src="https://sso.hcmut.edu.vn/cas/images/bk_logo.png" alt="">
                            <div class="slogan">Tech Food</div>
                        </div>
                        <div class="nav_bar_up_menu">
                            <ul class="list_menu">
                                <li>
                                    <img src="https://cdn-icons-png.flaticon.com/512/3075/3075977.png" alt="">
                                    <a href="/">Food</a>
                                </li>
                                <li>
                                    <img src="https://cdn-icons-png.flaticon.com/512/2738/2738730.png" alt="">
                                    <a href="/">Drink</a>
                                </li>
                                <li>
                                    <i class="bi bi-file-earmark-person"></i>
                                    <a href="/">About</a>
                                </li>
                            </ul>
                        </div>
                        <div class="nav_bar_up_cart">
                            <i class="bi bi-cart-check"></i>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="container_app">
            <div class="grid">
                <div class="grid__row">
                    <div class="col-9">
                        <div class="item_list">
                            <div class="grid__row">
                                <div class="col-3">
                                    <div class="item_list_detail" id="1">
                                        <img class="showModal"
                                            src="https://cdn.daylambanh.edu.vn/wp-content/uploads/2017/08/cach-lam-banh-hamburger-bo-600x400.jpg"
                                            alt="">
                                        <div class="item_list_content">
                                            <div class="item_name">Hambuger</div>
                                            <div class="item_quantity">Số lượng: 20</div>
                                            <div class="item_sale">
                                                <div class="item_price">Giá: <span>20000</span>đ</div>
                                                <button class="button_add_cart" id="1"><i
                                                        class="bi bi-cart"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-3">
                                    <div class="item_list_detail" id="2">
                                        <img class="showModal"
                                            src="https://cdn.tgdd.vn/2021/05/CookRecipe/Avatar/banh-mi-thit-bo-nuong-thumbnail-1.jpg"
                                            alt="">
                                        <div class="item_list_content">
                                            <div class="item_name">Bánh mì</div>
                                            <div class="item_quantity">Số lượng: 20</div>
                                            <div class="item_sale">
                                                <div class="item_price">Giá: <span>10000</span>đ</div>
                                                <button class="button_add_cart" id="2"><i
                                                        class="bi bi-cart"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-3">
                                    <div class="item_list_detail" id="3">
                                        <img class="showModal"
                                            src="https://image.thanhnien.vn/1024/uploaded/congthang/2019_10_09/anh_1_dzzf.jpg"
                                            alt="">
                                        <div class="item_list_content">
                                            <div class="item_name">Bún bò</div>
                                            <div class="item_quantity">Số lượng: 20</div>
                                            <div class="item_sale">
                                                <div class="item_price">Giá: <span>20000</span>đ</div>
                                                <button class="button_add_cart" id="3"><i
                                                        class="bi bi-cart"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-3">
                                    <div class="item_list_detail" id="4">
                                        <img class="showModal"
                                            src="https://thucthan.com/media/2018/06/bun-dau-mam-tom/cach-lam-bun-dau-mam-tom.jpg"
                                            alt="">
                                        <div class="item_list_content">
                                            <div class="item_name">Bún đậu</div>
                                            <div class="item_quantity">Số lượng: 20</div>
                                            <div class="item_sale">
                                                <div class="item_price">Giá: <span>45000</span>đ</div>
                                                <button class="button_add_cart" id="4"><i
                                                        class="bi bi-cart"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-3">
                                    <div class="item_list_detail" id="5">
                                        <img class="showModal"
                                            src="https://cdn.daynauan.info.vn/wp-content/uploads/2019/05/com-tam-la-mon-an-binh-dan.jpg"
                                            alt="">
                                        <div class="item_list_content">
                                            <div class="item_name">Cơm sườn 1</div>
                                            <div class="item_quantity">Số lượng: 20</div>
                                            <div class="item_sale">
                                                <div class="item_price">Giá: <span>20000</span>đ</div>
                                                <button class="button_add_cart" id="5"><i
                                                        class="bi bi-cart"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-3">
                                    <div class="item_list_detail" id="6">
                                        <img class="showModal"
                                            src="https://i.ytimg.com/vi/cJu6tFJe_Gc/maxresdefault.jpg" alt="">
                                        <div class="item_list_content">
                                            <div class="item_name">Cơm sườn 2</div>
                                            <div class="item_quantity">Số lượng: 20</div>
                                            <div class="item_sale">
                                                <div class="item_price">Giá: <span>20000</span>đ</div>
                                                <button class="button_add_cart" id="6"><i
                                                        class="bi bi-cart"></i></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-3">
                        <div class="item_cart">
                            <div class="header_cart">
                                <i class="bi bi-cart-check-fill"></i>
                                <div class="your_cart">Your Cart (<span>0</span>)</div>
                            </div>
                            <div class="item_cart_list">

                            </div>
                            <div class="payment_cart">
                                <button id="button_payment" class="button_payment">Thanh toán</button>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="footer">

        </div>
        <div class="modal_product displayNode">
            <div class="modal_content">
                <div class="modal_content_in">
                    <div class="content_top">
                        <div class="content_top_img">
                            <img src="https://i.ytimg.com/vi/cJu6tFJe_Gc/maxresdefault.jpg" alt="">
                        </div>
                        <div class="content_top_sale">
                            <h2 class="content_top_name">Cơm sườn 2</h2>
                            <div class="content_top_price">Price: 20000</div>
                            <div class="content_top_quantity">Số lượng: 20</div>
                        </div>
                    </div>
                    <div class="content_bottom">
                        <div class="content_bottom_description">
                            <h2 class="content_bottom_description_title">Description</h2>
                            <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Assumenda labore aliquid nihil
                                accusamus non. Rerum sunt necessitatibus deserunt impedit? Commodi voluptates laboriosam
                                tempora incidunt consectetur, totam cumque quisquam minus molestiae!</p>
                        </div>
                    </div>
                    <div class="button_close_modal">
                        <button id="button_close_modal">Close</button>
                    </div>

                </div>
            </div>
        </div>
    </div>
    <script src="./../../assets/js/foodjs.js"></script>
</body>

</html>